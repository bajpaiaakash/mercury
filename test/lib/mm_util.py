...
def generate_html_response(operation, obj, params=None):
    ...
    if operation == 'unit_test' or operation == 'test':
        ...
        html = template.render(result=result,results_normal={},args=params,base_path=config.base_path)
    elif operation == 'deploy':
...
def does_file_exist(api_name, metadata_type_name):
    ...
    if os.path.isfile(os.path.join(config.project_path,"src",metadata_type['directoryName'],api_name+"."+metadata_type['suffix'])):
        return True
    else:
        return False
...
def get_file_lines(api_name, metadata_type_name):
    try:
        metadata_type = get_meta_type_by_name(metadata_type_name)
        if os.path.isfile(os.path.join(config.project_path,"src",metadata_type['directoryName'],api_name+"."+metadata_type['suffix'])):
            return open(os.path.join(config.project_path,"src",metadata_type['directoryName'],api_name+"."+metadata_type['suffix'])).readlines()
        else:
            return []
    except:
        return []
